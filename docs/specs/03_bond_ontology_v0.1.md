...
* It constrains **what a Bond is in data and behavior**: a single object that is both **prompt + connector** (for operator bonds) or **hololoop connector** (for link-only bonds), with a strict lifecycle (draft → executed for operators) and auditable lineage (inputs → bond → output).
* It constrains **how Bonds are created**: user confirmation is required (including when the system suggests prompts or hololoop options).
* It constrains **how execution is logged**: execution must produce an output Item and append QDPIEvents (append-only).
* It ensures **bonds are navigational hololinks**, not generic templates or "link types" as user-facing labels.

Bond ontology does **not** constrain what users can write. Users can write any `prompt_text`, execute without any explicit type/label, and create "link-only" Bonds (hololoops) for Queue Lattice structure.
...
* **Bond:** A persistent instruction that connects one or more input Items. Bonds may be **link-only** (hololoops between Queue Items) or **operator bonds** (executable prompts that generate output Items). Operator bonds are created as `status:"draft"` and may be executed; execution generates exactly one output Item and MUST set `status:"executed"` and `output_item_id`. Link-only bonds remain `status:"draft"` and are never executed.
* **Queue:** An Item with `type:"Q"`. Queue Items are artifacts, not prompts. They form the foundation of the Queue Lattice.
...
**Bond (prompt + connector, or hololoop)**
A persistent instruction connecting one or more input Items via `input_item_ids`. Bonds come in two forms:

1. **Link-only Bond (hololoop):** Connects two Queue Items via hololinks. `prompt_text` contains forward and return hololink sentences. Remains `status:"draft"`, never executed.
2. **Operator Bond:** Stores a user-facing prompt in `prompt_text` and may be executed to generate exactly one output Item.

> **Critical rule:** For operator bonds, the Bond is the prompt. The user-facing thing is `prompt_text`. For link-only bonds, the Bond is the navigational hololoop. The visible bond sentence must be a navigational hololink, not a generic template.
...
## 5) Bond Forms (by purpose and execution)

### A) Queue Lattice Bonds (Q↔Q Hololoops)

* **Inputs:** exactly 2 Queue Items (`input_item_ids.length == 2`, both `type:"Q"`)
* **Primary user-facing content:** `prompt_text` contains forward and return hololink sentences
* **Execution:** never executed; remains `status:"draft"` with `output_item_id:null`
* **Primary use in v0.1:** build the Queue Lattice by connecting Queue Items via content-derived hololinks
* **Hololink requirement:** Each hololink must reference handles from both Items, not generic templates

### B) Operator Bonds (Q→M, Q→D)

* **Inputs:** 1+ Items (`input_item_ids.length >= 1`), typically including at least one Queue Item
* **Primary user-facing content:** `prompt_text` (executable prompt)
* **Execution:** creates **one new output Item** (usually `type: "M"` or `type: "D"`)
* **Lifecycle:** `status:"draft" → "executed"` when run
* **Primary use in v0.1:** the Guided Tutorial's Q→M (suggested prompt) and Q→D (user-written prompt)
* **Anchoring requirement:** Generated M/D Items must have an explicit lineage path back to a Queue Item via provenance

### C) Holologue Operator (many→one)

* **Inputs:** 2+ Items selected by the user (and the system may optionally consider nearby Bonds for context)
...
* **Proposal requirement:** Proposals should be hololink-like, derived from content handles, not generic templates unless source content demands it.
...
* Labels are **not** the primary UI. The visible bond sentence must be a navigational hololink (for Queue lattice bonds) or an executable prompt (for operator bonds).
* Labels are **not** user-facing link types. Users never see "this is a 'bridges' bond" or "this is a 'clarifies' bond" as the primary label.
...
This is the key UX: system proposes 4 candidate bond prompts from Item content. For Queue Lattice Mode, system proposes 4 hololoop options (each containing forward and return hololinks).

### When suggestions appear

**For Queue Lattice Mode (hololoop options):**

* After creating a second (or later) Queue Item, system presents 4 hololoop options to connect the new Queue to a target Queue.
* User must select exactly 1 → persists link-only Bond.
* The 3 unselected remain event-only proposals.

**For operator bonds (Q→M, Q→D):**
...
**For hololoop options:**

* Each option consists of forward and return hololink sentences.
* Hololinks must be content-derived, referencing handles from both Queue Items.
* Options should be meaningfully distinct (different handles, different angles).

**For operator bond prompts:**

* A candidate Bond prompt (a string) shaped by Item content.
* It may also carry an optional internal grammar label (one of the 12) for diversity and downstream reasoning.

### Hard requirements for the 4 prompts (operator bonds)
...
### Hard requirements for the 4 hololoop options (Queue Lattice)

* **Content-derived:** each hololink must reference at least one handle from the source Item and one from the target Item.
* **Navigational:** hololinks read like sentences a human would click to "go there" (8–22 words, specific).
* **Distinct:** options should use different handles and angles, not "same sentence with synonyms."
* **No generic templates:** avoid cookie-cutter scaffolds unless source content explicitly demands them.
...
## 10) Relationship to Queue Lattice and Guided Tutorial flows

Bond ontology directly supports Queue Lattice Mode and the three tutorial moves:

### Queue Lattice Mode (Q↔Q hololoops)

* User creates Queue Items (Q1, Q2, Q3, Q4...).
* After Q2 creation, system presents 4 hololoop options (`bond.suggestions.presented` with `mode:"queue_lattice"`).
* Each option contains forward and return hololink sentences, content-derived from both Items.
...
* Generated M Item must anchor back to Queue via provenance.
...
* Generated D Item must anchor back to Queue via provenance.
...
* Holologue proposals should be hololink-like, derived from content handles, not generic templates.

**Clarification:** Bond grammar labels are optional and mostly hidden; they may appear as subtle chips in a details panel but are never required to execute. The visible bond sentence must be a navigational hololink (for Queue lattice) or an executable prompt (for operators).
...
### Example 3 — Queue Lattice Hololoop (link-only, Q↔Q)
...
**QDPIEvent — system presents 4 hololoop options**

```json
{
  "schema_version": 1,
  "id": "ev_01JFBND03EV000000000000000",
  "network_id": "nw_01JFBND01NET00000000000001",
  "episode_id": "ep_01JFBND01EP000000000000001",
  "ts": "2025-12-16T00:02:15.000Z",
  "seq": 10,
  "qdpi": "Q",
  "direction": "system→field",
  "actor": { "kind": "system", "id": "system", "display": "Field-Kit" },
  ...
  "refs": {
    "mode": "queue_lattice",
    "source_item_id": "it_01JFBND03B00000000000000001",
    "target_item_id": "it_01JFBND03A00000000000000001",
    "suggestions": [
      {
        "forward_text": "If 'visible persistence' is required, does the Ledger's 'inputs → output' display satisfy that proof?",
        "return_text": "Does 'append-only ledger' imply that executed bonds must show lineage in the Ledger UI?",
        "handles_used": ["visible persistence", "Ledger", "inputs → output"]
      },
      {
        "forward_text": "Does 'qdpi tags' connect to 'object IDs' as a way to trace lineage?",
        "return_text": "If 'object IDs' are visible, do 'qdpi tags' help users understand the event flow?",
        "handles_used": ["qdpi tags", "object IDs", "lineage"]
      },
      {
        "forward_text": "Does 'append-only ledger' mean the Ledger must show events in order?",
        "return_text": "If events are ordered, does 'append-only' guarantee the Ledger shows persistence?",
        "handles_used": ["append-only ledger", "events", "persistence"]
      },
      {
        "forward_text": "Does 'executed bonds' require showing 'inputs → output' in the Ledger?",
        "return_text": "If 'inputs → output' is visible, does that prove 'executed bonds' are not chat?",
        "handles_used": ["executed bonds", "inputs → output", "not chat"]
      }
    ]
  }
}
...
**Bond (draft, link-only hololoop) — user selected option 1**
...
```json
{
  ...
  "prompt_text": "it_01JFBND03B00000000000000001→it_01JFBND03A00000000000000001: If 'visible persistence' is required, does the Ledger's 'inputs → output' display satisfy that proof?\nit_01JFBND03A00000000000000001→it_01JFBND03B00000000000000001: Does 'append-only ledger' imply that executed bonds must show lineage in the Ledger UI?",
  "status": "draft",
  "output_item_id": null,
  "created_by": "user",
...
* **Do not reduce Bonds to labels.**
  Labels (`intent_type`) are optional hints; the Bond is `prompt_text` (for operators) or hololink sentences (for Queue lattice).

* **Do not show "link types" as user-facing labels.**
  The visible bond sentence must be a navigational hololink (for Queue lattice) or an executable prompt (for operators). Link-type metadata can exist internally but must not be the primary UI.
...
* **Do not use generic templates for hololinks.**
  Hololinks must be content-derived, referencing handles from both Items. Avoid cookie-cutter scaffolds like "Expand X into a checklist" unless the source content explicitly demands it.
...
* **Do not conflate Credits with Bonds.** Credits are event-derived bookkeeping; Bonds remain prompts + connectors (or hololoops).
* **Do not allow M/D generation without Queue anchoring.** Generated M/D Items must have an explicit lineage path back to a Queue Item via provenance.
...
* [ ] Queue Lattice Mode is supported:

  * [ ] System presents 4 hololoop options after Q2+ creation.
  * [ ] Each option contains forward and return hololink sentences, content-derived from both Items.
  * [ ] User selects exactly 1 → persists link-only Bond.
  * [ ] Link-only Bonds remain `status:"draft"`, never executed.
* [ ] User confirmation is required to create a Bond draft (including from suggestions or hololoop options).
* [ ] System can propose **exactly 4** content-shaped Bond prompts on Item create/selection (for operator bonds).
...
* [ ] Link-only Bonds (hololoops) are supported: `input_item_ids.length === 2` (both Queue Items), `prompt_text` contains forward and return hololinks, `status:"draft"`, `output_item_id:null`, never executed.
* [ ] M/D Items anchor to Queue: Generated M/D Items MUST have at least one Queue Item in `provenance.input_item_ids`.
* [ ] Holologue produces one specific artifact Item (`type:"H"`) and may propose Bond prompts (events only).
* [ ] Hololinks are content-derived: must reference handles from both Items, not generic templates.
