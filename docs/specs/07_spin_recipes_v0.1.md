...
Spin Recipes are a small, reusable library of **copy/paste-ready prompt templates** that Field-Kit can use to (a) generate **hololink pairs** for Queue lattice bonding (Q↔Q hololoops), (b) suggest Bond prompts for **Q→M** (system-suggested operator Bond execution), (c) support **Q→D** (user-written operator Bond execution), (d) drive **Holologue** runs (many→one artifact creation), and (e) generate **proposal sets** (4 bond prompt suggestions as events only). Recipes exist to improve **repeatability, output quality, and diversity** while preserving the core rule: **hololinks and Bond `prompt_text` are primary and editable; recipes are defaults, not constraints.**
...
* **Item:** A persistent node of content in the Field, created by a user or generated by an operator. Items are addressable by ID and include a `type` (e.g., `Q` for Queue, `M` for Monologue, `D` for Dialogue, `H` for Holologue) plus content and minimal metadata.
* **Bond:** A persistent connection between Items. Bonds come in two forms: (1) **link-only bonds** (hololoops) connecting Queue Items with `bond_kind:"link"`, `status:"draft"`, never executed; (2) **operator bonds** (`bond_kind:"operator"`) that connect input Items to an operator run via `prompt_text`. Operator bonds are created as `status:"draft"` and may be executed; execution generates exactly one output Item and MUST set `status:"executed"` and `output_item_id`.
* **Hololink:** A one-way navigational sentence connecting two Items, derived from content handles. A hololink is a short sentence (typically 5–15 words) that describes how one Item relates to another.
* **Hololoop:** A two-way navigational loop formed by two hololinks (A→B and B→A), persisted as a link-only Bond.
...
* `{{handle_a}}` — a key noun/phrase extracted from source Item A (for hololinks)
* `{{handle_b}}` — a key noun/phrase extracted from target Item B (for hololinks)
...
## 6) Queue Lattice Hololink Recipes (Q↔Q) — 4

> These generate **4 hololoop options** (pairs of forward and return hololinks) for connecting Queue Items in the Queue Lattice. Each option consists of two short navigational sentences derived from content handles. The system presents 4 options as events only (`bond.suggestions.presented` with `mode:"queue_lattice"`); the user selects exactly 1 to persist as a link-only Bond.

---

### Recipe 1: `hololink_extends`

* **Name / ID:** `hololink_extends`
* **Intended move:** Q↔Q hololoop (link-only Bond)
* **Inputs (required):** `handle_a` (from source Queue Item), `handle_b` (from target Queue Item), `item_title_a`, `item_title_b`
* **Output contract:** Produces exactly **4 hololoop options**. Each option MUST include:
  * `link_text_forward` (A→B): 5–15 words, navigational sentence
  * `link_text_return` (B→A): 5–15 words, navigational sentence
  * Both hololinks MUST incorporate `{{handle_a}}` and `{{handle_b}}` verbatim or as close noun phrases
* **Constraints:** Hololinks must be distinct, readable (no ellipses), and feel like navigation, not templates. Avoid generic verbs like "relates to" without specificity.
* **Logging expectations (names only):** `bond.suggestions.presented` (with `mode:"queue_lattice"`)
* **Prompt template:**

  ```text
  Generate 4 distinct hololoop options connecting two Queue Items.
  Source Item A title: {{item_title_a}}
  Source Item A handle: "{{handle_a}}"
  Target Item B title: {{item_title_b}}
  Target Item B handle: "{{handle_b}}"

  Output JSON array of exactly 4 objects:
  [
    {
      "link_text_forward": "A→B sentence (5-15 words, includes handle_a and handle_b)",
      "link_text_return": "B→A sentence (5-15 words, includes handle_a and handle_b)"
    },
    ...
  ]

  Rules:
  - Each hololink must incorporate the handles verbatim or as close noun phrases
  - Hololinks must be navigational sentences, not generic templates
  - All 4 options must be meaningfully distinct
  - Forward and return links should form coherent two-way navigation
  ```
...
* **Example instantiation:**
  ...
  ```text
  Generate 4 distinct hololoop options connecting two Queue Items.
  Source Item A title: My First Field Item
  Source Item A handle: "prove persistence"
  Target Item B title: Second Field Item
  Target Item B handle: "ledger trail"

  Output JSON array of exactly 4 objects:
  [
    {
      "link_text_forward": "prove persistence requires showing ledger trail",
      "link_text_return": "ledger trail demonstrates persistence proof"
    },
    {
      "link_text_forward": "prove persistence extends into ledger trail verification",
      "link_text_return": "ledger trail anchors back to persistence proof"
    },
    {
      "link_text_forward": "prove persistence depends on ledger trail completeness",
      "link_text_return": "ledger trail validates persistence proof claims"
    },
    {
      "link_text_forward": "prove persistence connects to ledger trail inspection",
      "link_text_return": "ledger trail supports persistence proof demonstration"
    }
  ]
  ```
...
### Recipe 2: `hololink_contrasts`

* **Name / ID:** `hololink_contrasts`
* **Intended move:** Q↔Q hololoop (link-only Bond)
* **Inputs (required):** `handle_a`, `handle_b`, `item_title_a`, `item_title_b`
* **Output contract:** Produces exactly **4 hololoop options** with contrastive hololinks (e.g., "X differs from Y", "X complements Y").
* **Constraints:** Same as Recipe 1; hololinks emphasize contrast or complementarity.
* **Logging expectations (names only):** `bond.suggestions.presented` (with `mode:"queue_lattice"`)
* **Prompt template:**

  ```text
  Generate 4 distinct contrastive hololoop options connecting two Queue Items.
  Source Item A title: {{item_title_a}}
  Source Item A handle: "{{handle_a}}"
  Target Item B title: {{item_title_b}}
  Target Item B handle: "{{handle_b}}"

  Output JSON array of exactly 4 objects with contrastive hololinks (e.g., differs, complements, opposes, refines).
  Each hololink must incorporate handles and form coherent two-way navigation.
  ```

---

### Recipe 3: `hololink_sequences`

* **Name / ID:** `hololink_sequences`
* **Intended move:** Q↔Q hololoop (link-only Bond)
* **Inputs (required):** `handle_a`, `handle_b`, `item_title_a`, `item_title_b`
* **Output contract:** Produces exactly **4 hololoop options** with sequential hololinks (e.g., "X leads to Y", "X precedes Y").
* **Constraints:** Same as Recipe 1; hololinks emphasize sequence or causality.
* **Logging expectations (names only):** `bond.suggestions.presented` (with `mode:"queue_lattice"`)
* **Prompt template:**

  ```text
  Generate 4 distinct sequential hololoop options connecting two Queue Items.
  Source Item A title: {{item_title_a}}
  Source Item A handle: "{{handle_a}}"
  Target Item B title: {{item_title_b}}
  Target Item B handle: "{{handle_b}}"

  Output JSON array of exactly 4 objects with sequential hololinks (e.g., leads to, precedes, enables, follows).
  Each hololink must incorporate handles and form coherent two-way navigation.
  ```

---

### Recipe 4: `hololink_generalizes`

* **Name / ID:** `hololink_generalizes`
* **Intended move:** Q↔Q hololoop (link-only Bond)
* **Inputs (required):** `handle_a`, `handle_b`, `item_title_a`, `item_title_b`
* **Output contract:** Produces exactly **4 hololoop options** with generalization hololinks (e.g., "X is an instance of Y", "X exemplifies Y").
* **Constraints:** Same as Recipe 1; hololinks emphasize generalization or instantiation.
* **Logging expectations (names only):** `bond.suggestions.presented` (with `mode:"queue_lattice"`)
* **Prompt template:**

  ```text
  Generate 4 distinct generalization hololoop options connecting two Queue Items.
  Source Item A title: {{item_title_a}}
  Source Item A handle: "{{handle_a}}"
  Target Item B title: {{item_title_b}}
  Target Item B handle: "{{handle_b}}"

  Output JSON array of exactly 4 objects with generalization hololinks (e.g., is an instance of, exemplifies, generalizes, specializes).
  Each hololink must incorporate handles and form coherent two-way navigation.
  ```

---

## 7) Monologue Recipes (Q→M) — 8
...
## 9) Holologue Recipes (many→one) — 6
...
## 10) Proposal Generator Recipes — 2
...
## 11) Integration Notes (brief)

**Queue Lattice Mode**

* **Q↔Q hololoop creation:**
  * Use hololink recipes (e.g., `hololink_extends`, `hololink_contrasts`) to generate 4 hololoop options.
  * Options are emitted as `bond.suggestions.presented` with `mode:"queue_lattice"` (events only).
  * User selects exactly 1 → `bond.draft_created` (link-only Bond with `bond_kind:"link"`, `link_text_forward`, `link_text_return`).
  * Link-only Bonds remain `status:"draft"` and never execute.

**Guided Tutorial usage**

* **Q→M (suggested operator Bond execution):**
...
* [ ] A recipe registry exists (static config or code constants) containing:
  ...
  * [ ] 4 Queue Lattice hololink recipes (Q↔Q hololoops)
  * [ ] 8 Monologue recipes (Q→M)
  * [ ] 6 Dialogue recipes (Q→D)
  * [ ] 6 Holologue recipes (many→one H)
  * [ ] 2 proposal-generator recipes (4-pack prompts)
* [ ] Queue Lattice hololink generation enforces:

  * [ ] exactly 4 hololoop options per suggestion event
  * [ ] each option includes `link_text_forward` and `link_text_return` (both 5–15 words)
  * [ ] hololinks incorporate handles verbatim or as close noun phrases
  * [ ] hololinks are navigational sentences, not generic templates
  * [ ] all 4 options are meaningfully distinct
  * [ ] forward and return links form coherent two-way navigation
...
10. How should handle extraction work for Queue Items (title n-grams vs keyword extraction), and should it be deterministic to ensure consistent hololink generation?
11. Should Queue Lattice hololink recipes prefer a fixed diversity mapping (always include extends/contrasts/sequences/generalizes) or be purely content-driven based on Item similarity?