...
* **Draft (Item/Bond draft)**
  A persisted object that is still in-progress. In v0.1, this primarily applies to **Bond.status="draft"**. Link-only bonds (hololoops) remain `status:"draft"` and never execute. Operator bonds start as `status:"draft"` with `output_item_id:null` and become `status:"executed"` after successful execution. Items don't have a draft flag; they are workshop objects that may be curated (canonical) or not.
...
* **Link-only Bonds (hololoops): eligible for canon**

  * Link-only Bonds (`bond_kind:"link"`, `status:"draft"`, never executed) are committed workshop objects representing Queue lattice connections. They are eligible to become canonical if curated.
* **Draft operator Bonds: provisional**

  * Operator Bonds (`bond_kind:"operator"`, `status:"draft"` with `output_item_id:null`) are working/provisional. They are **not canonical by default** and are discouraged in curated views.
* **Executed operator Bonds + their output Items: committed (eligible for canon)**

  * Executed operator Bonds (`bond_kind:"operator"`, `status:"executed"`) and their output Items (`Item.provenance.created_by:"bond"`) are committed workshop objects and are eligible to become canonical if curated.
...
* **Link-only Bonds (hololoops):**
  ...
  * Required invariant: `bond_kind:"link"`, `status:"draft"`, `output_item_id:null` (never executed).
  * Editable: `link_text_forward`, `link_text_return`, `input_item_ids` (subject to validation: must be exactly 2 Queue Items), optional `intent_type`, `updated_at`.
  * Immutable: `bond_kind` (cannot change from "link" to "operator").
* **Operator Bonds while draft (`status:"draft"`):**

  * Required invariant: `bond_kind:"operator"`, `output_item_id:null`.
  * Editable: `prompt_text`, `input_item_ids` (subject to validation), optional `intent_type`, `updated_at`.
* **Operator Bonds after execution (`status:"executed"`):**

  * Required invariant: `bond_kind:"operator"`, `output_item_id` is set (non-null).
...
* **Failures:**
  ...
  * A failed operator run leaves the Bond as `status:"draft"` with `last_error` set; there is no "failed" status in v0.1.
...
* Link-only Bonds (hololoops) are **allowed** in curated views if explicitly listed in `episode.curated_bond_ids`; they represent Queue lattice structure.
* Draft operator Bonds (`bond_kind:"operator"`, `status:"draft"`) are **discouraged** but **allowed only if explicitly listed** in `episode.curated_bond_ids`. (They are never included implicitly.)
...
2. **Curated Bonds**
   ...
   * Link-only Bonds (hololoops) appear if explicitly listed.
   * Draft operator Bonds appear only if explicitly listed (warn if draft operator).
   * Ignore missing or archived IDs (warn).
...
* For Items created by operator Bond: show badge **"Derived from Bond bd_…"** and optionally **"from N inputs"**.
* For Holologue outputs: show badge **"Holologue artifact (from N items)"** and expose `artifact_kind`.
* For link-only Bonds (hololoops): show badge **"Queue lattice link"** or display the hololink text (`link_text_forward` / `link_text_return`) in the curated view.
...
* [ ] Bonds enforce lifecycle rules:
  ...
  * [ ] Link-only Bonds (`bond_kind:"link"`) remain `status:"draft"` with `output_item_id:null` and are editable
  * [ ] Operator Bonds (`bond_kind:"operator"`) start as `status:"draft"` with `output_item_id:null` and are editable
  * [ ] Executed operator Bonds (`bond_kind:"operator"`, `status:"executed"`) have non-null `output_item_id`
  * [ ] Executed operator bonds' `prompt_text` and `input_item_ids` are immutable
  * [ ] Link-only bonds' `bond_kind` is immutable
...
* [ ] Curated view can display hololoop bonds:

  * [ ] Link-only Bonds (hololoops) can be included in `episode.curated_bond_ids`
  * [ ] Curated view displays hololink text (`link_text_forward` / `link_text_return`) for hololoop bonds
