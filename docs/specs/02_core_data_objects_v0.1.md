...
These core objects define the **persistent Field** for v0.1: a Blank Field where users create Queue Items, build the Queue Lattice via hololoop Bonds, and execute operator Bonds to generate **specific artifact Items** (general→specific). Together, these objects enable: (1) durable state (Items/Bonds/Episodes/Networks), (2) **lineage/provenance** for generated outputs, (3) the operator loop (Bond execution and Holologue runs), and (4) an append-only **Ledger** via immutable QDPIEvents for audit, replay, and debugging.
...
* **Queue-first:** Creating an Item mints a Queue artifact (`type:"Q"`). Queue Items are artifacts, not prompts.
* **Append-only events are the audit trail:** QDPIEvent is immutable; corrections are new events.
* **Bonds are prompt + connector (one object):** a Bond stores inputs, prompt text (or hololink text for link-only bonds), execution status, and output linkage.
* **Link-only Bonds (hololoops):** Bonds connecting Queue Items via hololinks remain `status:"draft"` and are never executed. They represent navigational structure in the Queue Lattice.
* **Operator Bonds:** Bonds that execute to generate output Items. They transition `status:"draft" → "executed"` and set `output_item_id`.
* **Bond execution produces lineage:** executed Bonds must point to an output Item, and output Items must reference their origin.
* **M/D must anchor to Queue:** Generated M/D Items must have an explicit lineage path back to a Queue Item via provenance.
...
// Q = Queue (artifact, not prompt)
// M = Monologue (operator-generated output)
// D = Dialogue (operator-generated output from user-authored Bond)
// H = Holologue (many→one artifact)
...
A Bond is a **user-facing object** that serves one of two roles:

1. **Link-only Bond (hololoop):** A navigational connector between two Queue Items, formed by selecting hololinks in both directions. Link-only Bonds remain `status:"draft"` and are never executed. They represent structure in the Queue Lattice.
2. **Operator Bond:** A prompt + connector that can be **executed** to generate a new output Item. Operator Bonds have lifecycle `draft → executed`. Executed Bonds MUST set `output_item_id`.

Both types use the same Bond schema. The distinction is determined by:
- **Link-only:** `input_item_ids.length === 2`, both Items are `type:"Q"`, `prompt_text` contains forward and return hololink sentences, `status:"draft"`, `output_item_id:null`, never executed.
- **Operator:** May have 1+ input Items, `prompt_text` is an executable prompt, lifecycle `draft → executed`.
...
export type Bond = {
  ...
  // For link-only Bonds (hololoops): prompt_text contains two lines:
  //   "A→B: <forward_hololink_text>\nB→A: <return_hololink_text>"
  // For operator Bonds: prompt_text is an executable prompt string
...
* **Link-only Bonds (hololoops):**
  ...
  * `input_item_ids.length === 2` (both must be Queue Items, `type:"Q"`)
  * `prompt_text` MUST contain two lines separated by `\n`:
    * First line: `"<item_id_1>→<item_id_2>: <forward_hololink_text>"`
    * Second line: `"<item_id_2>→<item_id_1>: <return_hololink_text>"`
  * `status` MUST be `"draft"`
  * `output_item_id` MUST be `null`
  * Link-only Bonds MUST never be executed (no transition to `status:"executed"`)
* **Operator Bonds:**

  * `prompt_text` is an executable prompt string (not hololink format)
  * If `status === "draft"`:
    * `output_item_id` MUST be `null`
...
* **Created:** user selects 1 of 4 hololoop options, system persists Bond with `status:"draft"`, `output_item_id:null`, `prompt_text` containing forward and return hololinks.
* **Never executed:** link-only Bonds remain `status:"draft"` permanently.
...
**Example 1 — Link-only Bond (hololoop) between two Queue Items**
...
```json
{
  ...
  "prompt_text": "it_01JFA3KB2Q4P6D4X9H2ZK8N3AA→it_01JFA3KQ9C2L1M7V8B0N3H5J2PP: If the Author is unknown, is the 'collector' detective or suspect?\nit_01JFA3KQ9C2L1M7V8B0N3H5J2PP→it_01JFA3KB2Q4P6D4X9H2ZK8N3AA: Does 'Scheherazade in reverse' reframe the found text as evidence?",
  "status": "draft",
  "output_item_id": null,
  "created_by": "user",
  "created_by_actor": { "kind": "user", "id": "local-user", "display": "You" },
  "created_at": "2025-12-16T00:00:15.120Z",
  "updated_at": "2025-12-16T00:00:15.120Z"
}
```

**Example 2 — Draft Operator Bond created from a suggested prompt**
...
**Example 3 — Executed Operator Bond (latest output linked)**
...
## 8) Link-only Bonds (Hololoops) — Storage Options

### Option A (Recommended, minimal schema change)

Represent hololoops in `Bond.prompt_text` with two lines:

* `prompt_text` format: `"<item_id_1>→<item_id_2>: <forward_hololink_text>\n<item_id_2>→<item_id_1>: <return_hololink_text>"`
* `input_item_ids` contains exactly 2 Queue Items
* `status:"draft"`, `output_item_id:null`
* Never executed

**Advantages:** No schema changes, v0.1-compatible, simple to implement.

**Disadvantages:** Parsing required to extract forward/return hololinks, less explicit.

### Option B (Alternative, explicit fields)

Add optional fields for link-only bonds:

```ts
export type Bond = {
  // ... existing fields ...
  
  // Optional fields for link-only bonds (hololoops)
  bond_kind?: "link" | "operator";  // OPTIONAL (default inferred from usage)
  link_text_forward?: string;        // OPTIONAL (forward hololink sentence)
  link_text_return?: string;         // OPTIONAL (return hololink sentence)
...
**Rules:**
* If `bond_kind === "link"`: `link_text_forward` and `link_text_return` are REQUIRED, `input_item_ids.length === 2`, `status:"draft"`, `output_item_id:null`, never executed.
* If `bond_kind === "operator"` or unset: treat as operator bond, `prompt_text` is executable prompt.

**Advantages:** Explicit separation, easier querying, clearer semantics.

**Disadvantages:** Requires schema migration, adds optional fields.

**v0.1 recommendation:** Use Option A for minimal changes. Option B may be considered for v0.2+.

## 9) Provenance + Lineage Model (minimal, explicit)

### Bond-derived output Item

* **Operator Bond is the single user-facing object** that stores both structural linkage and prompt text.
* When an operator Bond execution succeeds:
...
* **M/D Items must anchor to Queue:** Generated M/D Items MUST have at least one Queue Item in their `provenance.input_item_ids` (the Queue that the operator executed on).
...
## 10) Event Sourcing Rules (explicit)
...
## 11) Minimal Storage Mapping (MVP)
...
## 12) Versioning + Migration Strategy
...
## 13) Security/Privacy Notes (minimal)
...
* [ ] Bonds support both link-only (hololoops) and operator bonds:

  * [ ] Link-only Bonds: `input_item_ids.length === 2` (both Queue Items), `prompt_text` contains forward and return hololinks, `status:"draft"`, `output_item_id:null`, never executed.
  * [ ] Operator Bonds: `input_item_ids` and `prompt_text` are required, lifecycle `draft → executed`.
...
* [ ] M/D Items anchor to Queue:

  * [ ] Generated M/D Items MUST have at least one Queue Item in `provenance.input_item_ids`.
* [ ] QDPIEvent is append-only and immutable:
