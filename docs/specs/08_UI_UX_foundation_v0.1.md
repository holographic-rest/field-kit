...
* Primary button: **"Create anything"** (creates Queue Item only; no type dropdown)
* Secondary button/link: **"Start Guided Tutorial"** (optional)
...
* Selecting one or more Items may open the operator drawer in **suggestions/proposals** mode (for operator bonds).
* Selecting Items must not steal focus from active text input elsewhere.
* Queue Items (Q) may show hololoop suggestions directly under the Item card (Queue Lattice Mode).
...
* Create flow: after saving a new Queue Item via **Create anything**, if Queue count >= 2, show hololoop suggestions under the Item (or in operator drawer for Queue Lattice Mode). After Queue count >= 4, operator suggestions unlock.
* Selection: selecting Items opens operator suggestions/proposals (for operator bonds).
...
**Queue Lattice Mode (Queue Items only):**

* After Q2 creation, hololoop suggestions appear directly under the Queue Item card (or in a clean panel).
* Hololink text must be **readable** (no ellipses truncation; full sentences visible).
* Each hololoop option shows:
  * Forward hololink text (A→B)
  * Return hololink text (B→A)
  * Action: **"Create Bond"** (persists link-only Bond)
* After Q4 hololoop, operator suggestions unlock (Q→M, Q→D, Holologue).
...
### 7.5 Hololoop suggestion cards (Queue Lattice Mode)

* Render directly under Queue Item cards (or in a clean panel)
* Each card contains:

  * Forward hololink text (A→B): **readable full sentence** (no ellipses)
  * Return hololink text (B→A): **readable full sentence** (no ellipses)
  * Primary action: **"Create Bond"** (persists link-only Bond with `bond_kind:"link"`)
* Exactly 4 options presented per `bond.suggestions.presented` event (with `mode:"queue_lattice"`).
* User selects exactly 1 → persists link-only Bond; 3 unselected remain event-only.
* Hololinks are **navigational sentences**, not generic templates.

### 7.6 Proposal cards (operator suggestions/proposals mode)

* Render inside operator drawer (for operator bonds)
* Each card contains:
...
### 7.8 Refinement chips (prompt helpers)
...
### 7.9 Run state panel (operator drawer)
...
### 8.1 Core loop (Queue Lattice Mode)

**Create Queue Item → (optional) select hololoop → (after Q4) pick/write operator prompt → Run → new Item appears**

1. User clicks **"Create anything"**
2. Composer opens (v0.1 default: modal)

   * **No type dropdown** (always creates Queue Item `type:"Q"`)
   * Enter content
   * Save
3. The new Queue Item appears at the **bottom** of the Field; auto-scroll brings it into view
4. **Queue Lattice Mode:**
   * After Q2 creation: system presents 4 hololoop options directly under the Item (or in clean panel)
   * Hololink text is **readable** (no ellipses; full sentences)
   * User selects exactly 1 → persists link-only Bond
   * After Q4 hololoop: operators unlock (Q→M, Q→D, Holologue)
5. **Operator mode (after Q4):**
   * Operator drawer opens in suggestions/proposals mode (non-blocking)
   * User either:
     * ignores suggestions and continues creating Queue Items, or
...
### 8.2 Queue Lattice bonding (hololinks under items)

**Trigger:** after creating Queue Item Q2, Q3, Q4...

* Hololoop suggestions appear directly under the Queue Item card (or in a clean panel)
* Each option shows forward and return hololink text (readable, no ellipses)
* User selects exactly 1 → `bond.draft_created` (link-only Bond with `bond_kind:"link"`)
* The 3 unselected options remain event-only (logged in `bond.suggestions.presented`)
* Bonding and navigation feel like **"hololinks under items"**, not generic templates

### 8.3 Tap Item → operator suggestions/proposals (Proposal-first)

**Trigger:** user selects one or more Item cards (after operators unlock)

* Operator drawer opens in suggestions/proposals mode showing:

  * Suggestions for the current selection (operator bonds)
...
### 8.4 Create Bond (explicit confirmation)
...
### 8.6 Run Holologue (many → one Bundle)
...
### 8.7 Open Ledger (inspectability-first)
...
* User saves a new Queue Item via **"Create anything"**

  * Event: `item.created`
  * qdpi: `Q`
  * direction: `user→field`
  * refs: `item_id`, `episode_id`, `type:"Q"`
...
* Queue Lattice Mode: after Q2 creation, system presents 4 hololoop options

  * Event: `bond.suggestions.presented` (with `mode:"queue_lattice"`)
  * qdpi: `Q`
  * direction: `system→field`
  * refs: `episode_id`, source `item_id`, target `item_id`, hololoop options (forward/return hololinks)

* User selects Items (or completes Create Item) and the system shows operator suggestions

  * Event: `bond.suggestions.presented` (for operator bonds)
  * qdpi: `Q`
  * direction: `system→field`
...
* User confirms **Create Bond** from a hololoop option (Queue Lattice)

  * Event: `bond.draft_created`
  * qdpi: `D`
  * direction: `user→field`
  * refs: `bond_id`, `episode_id`, input `item_id`s (2 Queue Items), `bond_kind:"link"`

* User confirms **Create Bond** from an operator Proposal

  * Event: `bond.draft_created`
  * qdpi: `D`
  * direction: `user→field`
  * refs: `bond_id`, `episode_id`, input `item_id`s, `bond_kind:"operator"`
...
* [ ] Primary CTA is **"Create anything"** (creates Queue Item only; no type dropdown)
* [ ] Tutorial entry includes **"Start Guided Tutorial"** (optional)
...
* [ ] After Q2 creation, hololoop suggestions appear directly under Queue Item cards (or in clean panel)
* [ ] Hololink text is **readable** (no ellipses truncation; full sentences visible)
* [ ] Each hololoop option shows forward hololink (A→B) and return hololink (B→A)
* [ ] User selects exactly 1 → persists link-only Bond (`bond_kind:"link"`)
* [ ] After Q4 hololoop, operators unlock (Q→M, Q→D, Holologue)
* [ ] Bonding and navigation feel like **"hololinks under items"**, not generic templates
* [ ] No type dropdown at Item creation (always creates Queue Item)
